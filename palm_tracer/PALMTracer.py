...
class PALMTracer:
	...
	def hr(self) -> tuple[np.ndarray, np.ndarray]:
		...
		bg_color = round(s["Background"].value * MAX_UI_16 / 100)
		...
		if s["Type"].value == 0:
			...
			if s["Dimension"].value == 0:  # .																				   Rendu 2D
				...
				viz = self._renderer.localizations(viz_data, color_mode, bg_color, gaussian)  # .							   Rendu
			else:  # .																										   Rendu 3D
				...
				if s["Dimension"].value == 1:  # .																			   Rendu Z Stack
					...
					viz = self._renderer.z_stack(viz_data, color_mode, z_step if z_step != 0 else uniform_z_step, bg_color, gaussian)  # Rendu
				else:  # .																									   Rendu 3D Rotation
					frames = s.hr_3d["Frames"].value
					axis = s.hr_3d["Axis"].value
					viz = self._renderer.rotation_3d(viz_data, color_mode, uniform_z_step, frames, axis, bg_color, gaussian)  # Rendu
