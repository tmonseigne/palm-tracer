...
from palm_tracer.Processing.DLL.Parsing import N_SEGMENT, parse_palm_result
...
class PalmCPU:
	...
	##################################################
	@staticmethod
	def __get_max_points(height: int = 256, width: int = 256, density: float = 0.2, n_planes: int = 1) -> int:
		"""
	...
	##################################################
	def __get_args(self, image: np.ndarray, threshold: float, watershed: bool, gauss_fit: int,
		...
		n = self.__get_max_points(height, width)  # Récupération d'un nombre de points maximum théorique
	...
	##################################################
	def run_image(self, image: np.ndarray, threshold: float, watershed: bool, gauss_fit: int,
	...
	##################################################
	def run_stack(self, stack: np.ndarray, threshold: float, watershed: bool, gauss_fit: int,
		...
		results = []
		for i in planes:
			points = self.run_image(stack[i], threshold, watershed, gauss_fit, sigma, theta, roi_size, i + 1)
	...
	def auto_threshold(self, image: np.array, roi_size: int = 7, max_iterations: int = 4):
		...
		for _ in range(max_iterations):
			...
			points = self.run_image(image, std_dev, False, 0, 1, math.pi / 4.0, roi_size)
